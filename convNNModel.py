    ...
    def trainModel(self, x_train, y_train, numIterations):
        ...
        for t in range(numIterations):
            ...
            if(self.previousIterationMeanLoss >= self.currentIterationMeanLoss):
                ...
                print("Model loss started increasing at iteration ",t)
            ...
            for i in range (self.sudokuGridSize):
                for j in range (self.sudokuGridSize):
                    ...
                    self.modelLoss[i][j] = self.sudokuConvNN[i][j].currentLoss
    ...
    def testModel(self, x_test, y_test):
        ...
        for i in range(self.sudokuGridSize):
            for j in range(self.sudokuGridSize):
                ...
                predictions[:,self.sudokuGridSize*i + j] = (np.argmax(A3,1, keepdims=False)+1).astype(int) #argmax() returns the INDEX of the value with the highest probability, which we increase by 1 to get the corresponding sudoku answer
